...
class SkeletonMarkers():
    def __init__(self):
        rospy.init_node('markers_from_tracker')
                
        rospy.loginfo("Initializing Skeleton Markers Node...")
        
        rate = rospy.get_param('~rate', 20)
        r = rospy.Rate(rate)
        tf_prefix = rospy.get_param('~tf_prefix', 'tracker')
        skeleton_frames = rospy.get_param('~skeleton_frames', '')
        self.fixed_frame = rospy.get_param('~fixed_frame', 'tracker_depth_frame')
        
        # Normalize the tf_prefix variable with / at the end
        tf_prefix = tf_prefix + '/'
        ...
        marker_pub = rospy.Publisher('skeleton_markers', Marker, queue_size=10)
        ...
        tf_pattern = re.compile('^' + tf_prefix + 'user_([0-9]+)/(\w+)$')
        ...
        while not rospy.is_shutdown():
            ...
            user_frames = [s for s in all_frames if tf_pattern.search(s)]
